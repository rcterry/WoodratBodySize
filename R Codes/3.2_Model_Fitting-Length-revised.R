...
# Fit Models - LENGTH
# -------------------------
fig_dir <- paste0(output_dir, "/3-Allometries/Length/")
combined_pdf <- paste0(fig_dir, "3.2-Allometries-Length-Combined.pdf")
...
for (s in 1:length(species))  {
  ...
  for (m in 1:length(measures)) {
    xy <- cbind(temp[,measures[m]],temp[, 'Length'])
    xy_clean <- xy[complete.cases(xy), ]
    x <- xy_clean[,1]
    y <- xy_clean[,2]
    
    main = paste0("Maxilla-Length-",species[s],"-",measures[m],sep="")
...
write.csv(Max.Models,paste0(output_dir, "/3.2-Models-Maxilla-Length.csv"),row.names=F)

##############################
# NLS vs LS Model Performance

sink(paste0(output_dir, "/3.2-Maxilla-NLS_vs_LS-Model_Estimates-Length.txt"))
cat("a\n")
...
pdf(paste0(output_dir, "/3.2-Maxilla-NLS_vs_LS-Model-PE-Length.pdf"),height=4,width=5)
...
pdf(paste0(output_dir, "/3.2-Maxilla-NLS_vs_LS-Model-R2-Length.pdf"),height=4,width=5)
...
for (s in 1:length(species))  {
  ...
  for (m in 1:length(measures)) {
    ...
    main = paste0("Mandible-Length-",species[s],"-",measures[m],sep="")
...
# ~~~~~ Output data file with model fits ~~~~~~~~
write.csv(Max.Models,paste0(output_dir, "/3.2-Models-Mandible-Length.csv"),row.names=F)

##############################
# NLS vs LS Model Performance

sink(paste0(output_dir, "/3.2-Mandible-NLS_vs_LS-Model_Estimates-Length.txt"))
cat("a\n")
print(t.test(Max.Models$nls.a,Max.Models$ls.a))
cat("\n")
cat("b\n")
...
pdf(paste0(output_dir, "/3.2-Mandible-NLS_vs_LS-Model-PE-Length.pdf"),height=4,width=5)
plot(Max.Models$nls.pe,Max.Models$ls.pe,
...
pdf(paste0(output_dir, "/3.2-Mandible-NLS_vs_LS-Model-R2-Length.pdf"),height=4,width=5)
plot(Max.Models$nls.r2,Max.Models$ls.r2,
     main = "Mandibular Models",
     xlab = "NLS R^2",
     ylab = "LS R^2",
